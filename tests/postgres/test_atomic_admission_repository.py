...
def test_migrations_through_0005_and_exact_admission_function_inventory() -> None:
    ...
    assert EXPECTED_MIGRATION_VERSION == 5
...
def test_migrations_upgrade_immutable_0002(
    ...
    assert MigrationRunner(migration_dsn, migrations).apply() == [3, 4, 5]
...
def test_quota_usage_date_is_utc_even_when_session_timezone_crosses_date(
    ...
    ids, context = _seed_authority(admin_dsn, quotas='{"search":1}')
    timezone_name = (
        "Pacific/Kiritimati" if datetime.now(UTC).hour >= 10 else "America/Adak"
    )
