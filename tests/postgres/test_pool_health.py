    ...
    try:
        ...
        with psycopg.connect(admin_dsn) as connection:
            ...
            connection.execute(
                ...
                "VALUES (3,'unexpected',%s,'pending',false)",
                ("0" * 64,),
            )
        readiness = check_readiness(database, expected=load_migrations())
        assert readiness.ok is False
        assert readiness.unexpected_versions == (3,)
        assert readiness.non_applied_versions == (3,)
        with psycopg.connect(admin_dsn) as connection:
            connection.execute("SET ROLE medawarcre_migration")
            connection.execute(
                "DELETE FROM medawarcre.schema_migrations WHERE version=3"
            )
            connection.execute(
